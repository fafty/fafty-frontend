      ...
      {/* container  */}
        <div className="mx-auto px-8">
          <div className="flex justify-between items-center py-4 md:justify-start align-center">
            ...
            <Popover.Group
              as="nav"
              className="hidden md:flex space-x-10 mr-0 md:mr-auto"
            >
              <Link href={'/bundles'}>
                  ...
                  <nav className="grid gap-y-8">
                    {pagesLinks.map((item) => (
                      <Link href={item.href} key={item.href}>
                        <a className="-m-3 px-2 py-3 flex items-center rounded-md w-full font-medium text-gray-100 hover:bg-gray-500 focus:outline-nonedark:hover:bg-stone-600">
                          <item.icon
                            className="flex-shrink-0 h-6 w-6 text-slate-900 dark:text-slate-50"
                            aria-hidden="true"
                          />
